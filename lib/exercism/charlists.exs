defmodule Username do
  def sanitize(username) do
    ...
    |> List.flatten()
    |> Enum.filter(fn char -> char in ?a..?z or char == ?_ end)
  end

end

IO.puts(Username.sanitize(~c"krueger"))
IO.puts(Username.sanitize(~c"cäcilie_weiß"))
IO.puts(Username.sanitize(~c"mark_fischer$$$"))
