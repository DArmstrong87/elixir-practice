defmodule FreelancerRates do
  ...
  def apply_discount(before_discount, discount) do
    ...
    before_discount - (before_discount * discount_perc)
  end

  def monthly_rate(hourly_rate, discount) do
    daily_rate = daily_rate(hourly_rate)
    monthly_rate = daily_rate * 22
    discounted_rate = Float.ceil(apply_discount(monthly_rate, discount), 0)
    trunc(discounted_rate)
  end

  def days_in_budget(budget, hourly_rate, discount) do
    daily_rate = monthly_rate(hourly_rate, discount) / 22
    days = budget / daily_rate
    Float.floor(days, 1)
  end
end

IO.puts(FreelancerRates.daily_rate(100))
IO.puts(FreelancerRates.apply_discount(1000, 20))
IO.puts(FreelancerRates.monthly_rate(100, 20))
IO.puts(FreelancerRates.days_in_budget(2000, 100, 20))
