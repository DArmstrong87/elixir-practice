defmodule HighScore do
  ...
  def add_player(scores, name, score \\ @score_reset) do
  ...
  def reset_score(scores, name) do
    Map.put(scores, name, @score_reset)
  end
...
IO.puts(HighScore.new() |> inspect())
IO.puts(HighScore.add_player(HighScore.new(), "Dave Thomas") |> inspect())
IO.puts(HighScore.add_player(HighScore.new(), "José Valim", 486_373) |> inspect())
IO.puts(HighScore.remove_player(HighScore.new(), "Dave Thomas") |> inspect())
IO.puts(HighScore.reset_score(HighScore.new(), "José Valim") |> inspect())
IO.puts(HighScore.update_score(HighScore.new(), "José Valim", 5) |> inspect())
IO.puts(HighScore.get_players(HighScore.new()) |> inspect())
